...
public class BuscadorUF {
        ...
        @WebMethod(action = "buscarUFsCapital")
        public ListaUF buscarUFsCapital(@WebParam(name = "buscaCapital") String buscaCapital) throws Exception {
        	ListaUF ufs = new ListaUF();
        	
        	for (String uf[]: listaUFs) {
        		if (uf[1].toLowerCase().contains(buscaCapital.toLowerCase())) {
        			ufs.getUFs().add(new UF(uf[0], uf[1], uf[2]));
        		}
        	}
        ...
        @WebMethod(action = "buscarUFsHabitantes")
        public ListaUF buscarUFsHabitantes(@WebParam(name = "buscaHabitantes") String buscaHabitantes) throws Exception {
        	ListaUF ufs = new ListaUF();
        	
        	for (String uf[]: listaUFs) {
        		if (uf[2].contains(buscaHabitantes)) {
        			ufs.getUFs().add(new UF(uf[0], uf[1], uf[2]));
        		}
        	}
